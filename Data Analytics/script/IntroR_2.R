...
x <- c(1, 2, 4, 8, 16, 32)
x
x[-4] #!! non è assegnato ad un oggetto, devo fare
z <- x[-4]
z

z <- x[-c(4, 6)] #tolgo due elementi
z
...
x < 5
x >= 7 | x < 5
x[c(x >= 7 | x < 5)] #seleziono solo gli elementi true

x <- 0:7
all(x>0) #tutti soddisfano?
any(x<0) #almeno uno soddisfa?
...
x <- c(F, T, T, F)
y <- as.numeric(x) #così senza operatore aritmetico
z <- x*1

set.seed(123) #così si possono ottenere numeri "casuali" uguali agli altri
runif(3)
...
# genera una lista di 10 numeri da 1 a 10 in ordine casuale e dice quali
#indici sono i numeri minori di 4
...
#fa vedere solo quelli True,(come prima con la condizione), NA rimane così 
#(Not Available)
